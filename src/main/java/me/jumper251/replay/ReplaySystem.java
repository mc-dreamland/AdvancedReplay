...
public class ReplaySystem extends JavaPlugin {
	...
	public void onEnable() {
		...
		getLogger().info("Loading Replay v" + getDescription().getVersion() + " by " + String.join(",", getDescription().getAuthors()));
		
		ConfigManager.loadConfigs();
		ReplayManager.register();
		
		if (ConfigManager.USE_DATABASE || ConfigManager.USE_MONGODB) {
